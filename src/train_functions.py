...
def train_model(
    ...
    callbacks = [CustomEarlyStopping(**params["model"]["early_stopping"]), tf.keras.callbacks.TerminateOnNaN()]
    ...
    if logging:
        logdir = params.get("logs_path", Path("output") / "logs")
    ...
    model = RecommenderEngineModel(params, candidates= candidates, train_metrics = train_metrics)
...
def train_CV(candidates: tf.data.Dataset, params: dict, data_df: pd.DataFrame, features = list[str]):
    ...
    for fold, (train_index, val_index) in tqdm(enumerate(kf.split(data_df)), total=n_splits, desc='Cross Validation', unit='fold'):
        ...
        # Train the model
        model = train_model(candidates = candidates, train=train_ds, val=val_ds, params=params, verbose=0)
